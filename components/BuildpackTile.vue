      ...
      <div class="flex-auto">
        <div class="flex items-center">
          <h1 class="name ma0 condensed f4 fw4">
            <nuxt-link class="color-inherit hover-blue link no-underline" :to="{ name: 'buildpacks-id-version', params: { id: buildpack.id, version: latestVersion.name } }" v-if="latestVersion">
              {{buildpack.name}}
            </nuxt-link>
            <span v-else>
              {{buildpack.name}}
        ...
        <div class="flex" style="justify-content:space-between;">
          <div>
            ...
            <h2 class="fw4 f5 mb0 mt2">
              <nuxt-link class="latestversion blue link no-underline" :to="{ name: 'buildpacks-id-version', params: { id: buildpack.id, version: latestVersion.name } }">
                {{ latestVersion.name }}
              </nuxt-link>
            </h2>
          </div>
